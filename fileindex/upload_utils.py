...
def create_indexed_file_from_upload(
    ...
    derived_from: Optional[IndexedFile] = None,
    derived_for: Optional[str] = None,
    ...
    if isinstance(uploaded_file, TemporaryUploadedFile):
        ...
        file_name = default_storage.save(
            f"{path_prefix}/{uploaded_file.name}",
...
def validate_image_upload(
    file: Union[UploadedFile, Path, str],
            ...
            if min_dimensions:
                ...
                if width < min_w or height < min_h:
                    raise ValidationError(
                        ...
                        f"minimum required {min_w}x{min_h}"
                    )
...
def cleanup_failed_upload(file_path: Union[str, Path]) -> bool:
    """
