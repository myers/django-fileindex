...
class IndexedFileField(forms.FileField):
    ...
    def __init__(self, *args, allowed_extensions: Optional[List[str]] = None,
...
class MultipleIndexedFilesField(forms.FileField):
    ...
    def clean(self, data, initial=None):
        ...
        if self.max_files and len(files) > self.max_files:
            raise ValidationError(
                f"Too many files. Maximum allowed: {self.max_files}"
            )
        ...
        try:
            for file in files:
                ...
                if self.max_file_size and file.size > self.max_file_size:
                    raise ValidationError(
                        f"File '{file.name}' exceeds maximum size of {self.max_file_size} bytes"
                    )
